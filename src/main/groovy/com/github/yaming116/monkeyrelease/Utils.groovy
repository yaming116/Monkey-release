...
public class Utils {
    ...
    static def setApkName(project) {
        project.android.applicationVariants.all { variant ->
            ...
            def apk = new File(apkDir, fileName)
            if (variant.buildType.name != "debug") {//防止AS无法安装debug包(apk)
                variant.getPackageApplication().outputDirectory = new File(apk.getParent())
            }
            variant.getPackageApplication().outputScope.apkDatas.forEach { output ->
                output.outputFileName = apk.getName()
                if (variant.buildType.name == 'release') {
                    def apkPath = apk.getAbsolutePath()
